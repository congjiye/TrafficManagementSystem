    ...
    <div class="row">
        ...
        <div class="col-sm-8 blog-main">
            ...
            <c:forEach items="${subway}" var="subway">
                <div class="blog-post showBus" id="${subway.subwayId}">
                    <h2>${subway.subwayName} <small style="float: right;line-height: 33px;">换乘最少</small></h2>
                    <hr style="margin-top: 0px;">
                    <p style="font-family: '微软雅黑';">
                        上车: <small style="color: #9b9b9b;">${route.split("-")[0]}</small> 下车:<small style="color: #9b9b9b;">${route.split("-")[1]}</small>
                    </p>
                        <%--<a href="#" style="float: right;transform: translate(-5px,-20px);font-size: 16px">» 查看更多</a>--%>
                </div><!-- /.blog-post -->
            </c:forEach>
            <c:forEach items="${s}" var="s">
                <div class="blog-post showBus">
                    <h2><small>${s.split("-")[0]}→${s.split("-")[1]}</small></h2>
                    <hr style="margin-top: 0px;">
                    <p style="font-family: '微软雅黑';">
                        上车: <small style="color: #9b9b9b;"><a href="/desSubwayStation?stationName=${route.split("-")[0]}">${route.split("-")[0]}</a>
                    </small> 换乘:<small style="color: #9b9b9b;"><a href="/desSubwayStation?stationName=${s.split("-")[2]}">${s.split("-")[2]}</a></small>
                        下车:<small style="color: #9b9b9b;"><a href="/desSubwayStation?stationName=${route.split("-")[1]}">${route.split("-")[1]}</a></small>
                    </p>
                </div><!-- /.blog-post -->
            </c:forEach>
            <c:if test="${subway.size() == 0}">
                <c:if test="${s.size() == 0}">
                    地铁查询提醒您，没有换乘方案，也许是以下原因中的一个：
                    <hr style="border-top: 3px solid #00cec9;">
                    <ul>
                        <li>您要查询的路线要转三次车以上，本站只提供两次转车；</li>
                        <li>检查输入词是否有误，尝试其他的输入词</li>
                        <li><a href="/chooseSubway">重新查询</a></li>
                    </ul>
                </c:if>
            </c:if>
