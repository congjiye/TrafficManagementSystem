...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-sm-8 blog-main">
            ...
            <c:forEach items="${bus}" var="buslist">
                <div class="blog-post showBus" id="${buslist.busId}">
                    <h2>${buslist.busName} <small>路公交</small><small style="float: right;line-height: 33px;">换乘最少</small></h2>
            ...
            <c:forEach items="${s}" var="s">
                <div class="blog-post showBus">
                    ...
                    <hr style="margin-top: 0px;">
                    <p style="font-family: '微软雅黑';">
                        上车: <small style="color: #9b9b9b;"><a href="/desStation?stationName=${route.split("-")[0]}">${route.split("-")[0]}</a>
                    </small> 换乘:<small style="color: #9b9b9b;"><a href="/desStation?stationName=${s.split("-")[2]}">${s.split("-")[2]}</a></small>
                        下车:<small style="color: #9b9b9b;"><a href="/desStation?stationName=${route.split("-")[1]}">${route.split("-")[1]}</a></small>
                    </p>
                </div><!-- /.blog-post -->
            </c:forEach>
            <c:if test="${bus.size() == 0}">
                <c:if test="${s.size() == 0}">
                公交查询提醒您，没有换乘方案，也许是以下原因中的一个：
                <hr style="border-top: 3px solid #00cec9;">
                <ul>
                    <li>您要查询的路线要转三次车以上，本站只提供两次转车；</li>
                    <li>检查输入词是否有误，尝试其他的输入词</li>
                    <li><a href="/chooseBus">重新查询</a></li>
                </ul>
                </c:if>
            </c:if>
