...
<div class="container">
    <div class="blog-header">
        ...
        <p class="lead blog-description">公交详细信息显示</p>
    </div>
    <div class="row">
        ...
        <div class="col-sm-8 blog-main">
            <div class="row">
                <div class="col-sm-6" style="border-right: 5px solid #1dd1a1;">
                    <img src="../image/bus.jfif" class="img-thumbnail" style="width: 200px;height: 200px;">
                </div>
                <div class="col-sm-6">
                    ${bus.busName}路公交
                    <p>
                        <small>始发站-终点站:${route.routeName}</small>
                    </p>
                    <p>
                        <small>运行时间：${busDetails.busRunDate}</small>
                    </p><p>
                    <small>票价：${busDetails.busPay}元</small>
                </p><p>
                    <small>运行状态：${busDetails.busStatus}</small>
                </p><p>
                    <small>公司：${busDetails.busCompany}</small>
                </p>
                </div>
            </div>
            <br>
            <br>
            <blockquote>
                正常情况下,单程70分,间隔5-9分<span style="color: #e67e22;font-size: 14px;"> 棕色为可换乘</span>
            </blockquote>
            <h4><strong>${bus.busName}路线站点</strong>
                <c:catch var="IndexOutOfBoundsException">
                    <p style="display: none;">${station.get(station.size() - 1) != null}</p>
                </c:catch>
                <c:if test="${IndexOutOfBoundsException == null}">
                    <strong style="float: right;">共${station.get(station.size() - 1).busPostison}站</strong>
                </c:if>
                <c:if test="${IndexOutOfBoundsException != null}">
                    <strong style="float: right;">共0站</strong>
                </c:if>
            </h4>
            <hr style="background-color:#00cec9;height:2px;border:none">
            <div>
                <c:forEach items="${station}" var="station">
                    <div style="float: left;" class="divType">
                            <a href="/desStation?stationName=${station.stationName}" class="aType"><i class="iType">${station.busPostison}</i>
                                ${station.stationName}
                            </a>
                    </div>
                </c:forEach>
...
<script>
    ...
    function compare() {
        var arry = $(".divType a");
        arry.each(function (i,item) {
            var str = item.text.replace(/\s*/g,"").replace(/\d+/g,'');
            $.ajax({
                url:"/compareStation",
                data:{"station":str},
                type:"POST",
